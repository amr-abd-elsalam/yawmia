...
test('Phase 50: counter compaction prunes stale buckets and preserves platform totals', async () => {
  const tempDir = await mkdtemp(join(tmpdir(), 'yawmia-phase50-counter-'));
  process.env.YAWMIA_DATA_PATH = tempDir;

  try {
    const db = await import('../server/services/database.js?phase50counter=' + Date.now());
    await db.initDatabase();

    const counters = await import('../server/services/directOfferCounters.js?phase50counter=' + Date.now());
    const compaction = await import('../server/services/counterCompaction.js?phase50counter=' + Date.now());
...
test('Phase 50: counter compaction archives inactive entities idempotently', async () => {
  const tempDir = await mkdtemp(join(tmpdir(), 'yawmia-phase50-counter-archive-'));
  process.env.YAWMIA_DATA_PATH = tempDir;

  try {
    const db = await import('../server/services/database.js?phase50counterarchive=' + Date.now());
    await db.initDatabase();

    const counters = await import('../server/services/directOfferCounters.js?phase50counterarchive=' + Date.now());
    const compaction = await import('../server/services/counterCompaction.js?phase50counterarchive=' + Date.now());
