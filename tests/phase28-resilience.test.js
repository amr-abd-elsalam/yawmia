...
describe('Phase 28 — Health Endpoint', () => {
  ...
  it('P28-13: health version is 0.24.0', async () => {
    const content = await readFile(resolve('server/router.js'), 'utf-8');
    assert.ok(content.includes("version: '0.24.0'"), 'health version should be 0.24.0');
  });
});
...
describe('Phase 28 — Service Worker Cache', () => {
  ...
  it('P28-32: sw.js CACHE_NAME is yawmia-v0.24.0', async () => {
    const content = await readFile(resolve('frontend/sw.js'), 'utf-8');
    assert.ok(content.includes("'yawmia-v0.24.0'"), 'cache name should be yawmia-v0.24.0');
  });
});
...
describe('Phase 28 — Version', () => {
  ...
  it('P28-33: package.json version is 0.24.0', async () => {
    const raw = await readFile(resolve('package.json'), 'utf-8');
    const pkg = JSON.parse(raw);
    assert.strictEqual(pkg.version, '0.24.0');
  });

  it('P28-34: config PWA cacheName is yawmia-v0.24.0', () => {
