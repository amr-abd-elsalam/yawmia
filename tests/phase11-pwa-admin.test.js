...
describe('Phase 11 — Config', () => {
  ...
  it('P11-01: Config has 28 sections', () => {
...
describe('Phase 11 — PWA Files', () => {
  ...
  it('P11-13: sw.js exists and contains cache name', async () => {
    ...
    assert.ok(content.includes('yawmia-v0.14.0'), 'sw.js should contain cache name yawmia-v0.14.0');
  });
...
describe('Phase 11 — Version', () => {
  ...
  it('P11-19: package.json version is 0.13.0', async () => {
    const pkgPath = resolve('package.json');
    const raw = await readFile(pkgPath, 'utf-8');
    const pkg = JSON.parse(raw);
    assert.strictEqual(pkg.version, '0.14.0');
  });

  it('P11-20: Config cacheName matches version', () => {
    assert.strictEqual(config.PWA.cacheName, 'yawmia-v0.14.0');
  });
});
