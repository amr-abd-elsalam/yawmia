...
describe('Phase 11 — Config', () => {
  ...
  it('P11-01: Config has 24 sections', () => {
    const keys = Object.keys(config);
    assert.strictEqual(keys.length, 24, `expected 24 config sections, got ${keys.length}: ${keys.join(', ')}`);
  });
...
describe('Phase 11 — PWA Files', () => {
  ...
  it('P11-13: sw.js exists and contains cache name', async () => {
    ...
    assert.ok(content.includes('yawmia-v0.11.0'), 'sw.js should contain cache name yawmia-v0.11.0');
  });
...
describe('Phase 11 — Version', () => {
  ...
  it('P11-19: package.json version is 0.11.0', async () => {
  ...
  it('P11-20: Config cacheName matches version', () => {
    assert.strictEqual(config.PWA.cacheName, 'yawmia-v0.11.0');
  });
});
