...
describe('Phase 11 — Config', () => {
  ...
  it('P11-01: Config has 45 sections', () => {
    const keys = Object.keys(config);
    assert.strictEqual(keys.length, 45, `expected 45 config sections, got ${keys.length}: ${keys.join(', ')}`);
  });
...
describe('Phase 11 — PWA Files', () => {
  ...
  it('P11-13: sw.js exists and contains cache name', async () => {
    ...
    assert.ok(content.includes('yawmia-v0.29.0'), 'sw.js should contain cache name yawmia-v0.29.0');
  });
