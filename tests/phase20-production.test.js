...
describe('Phase 20 — Health & Docs (Source Check)', () => {
  ...
  it('P20-20: health handler version is 0.35.0', async () => {
    const content = await readFile(resolve('server/router.js'), 'utf-8');
    assert.ok(content.includes("version: '0.35.0'"), 'version should be 0.35.0');
  });
...
describe('Phase 20 — File Existence', () => {
  ...
  it('P20-25: sw.js CACHE_NAME is yawmia-v0.35.0', async () => {
    const content = await readFile(resolve('frontend/sw.js'), 'utf-8');
    assert.ok(content.includes("'yawmia-v0.35.0'"), 'cache name should be yawmia-v0.35.0');
  });
});
