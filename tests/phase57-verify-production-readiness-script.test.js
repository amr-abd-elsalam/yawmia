...
test('verify-production-readiness --json outputs machine-readable JSON', async () => {
  const dir = await mkdtemp(join(tmpdir(), 'yawmia-readiness-script-'));
  ...
  const proc = spawnSync(process.execPath, ['scripts/verify-production-readiness.js', '--json'], {
  ...
  assert.ok(data.checks.some(c => c.id === 'queue_health'));
