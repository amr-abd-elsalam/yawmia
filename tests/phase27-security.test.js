...
describe('Phase 27 — Version', () => {
  ...
  it('P27-32: config PWA cacheName is yawmia-v0.23.0', () => {
    assert.strictEqual(config.PWA.cacheName, 'yawmia-v0.23.0');
  });

  it('P27-33: sw.js CACHE_NAME is yawmia-v0.23.0', async () => {
    const content = await readFile(resolve('frontend/sw.js'), 'utf-8');
    assert.ok(content.includes("'yawmia-v0.23.0'"), 'sw.js cache name should be yawmia-v0.23.0');
  });

  it('P27-34: health endpoint version is 0.23.0', async () => {
    const content = await readFile(resolve('server/router.js'), 'utf-8');
    assert.ok(content.includes("version: '0.23.0'"), 'router health version should be 0.23.0');
  });
});
...
describe('Phase 27 — Route Count', () => {
  ...
  it('P27-35: Router still has 70 routes (unchanged)', async () => {
    const content = await readFile(resolve('server/router.js'), 'utf-8');
