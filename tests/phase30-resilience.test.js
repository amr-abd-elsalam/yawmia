...
describe('Phase 30 — Version', () => {
  ...
  it('P30-61: sw.js CACHE_NAME is yawmia-v0.27.0', async () => {
    ...
    assert.ok(src.includes("'yawmia-v0.27.0'"), 'sw.js should use v0.27.0 cache name');
  });
  ...
  it('P30-62: config has 43 sections', async () => {
    ...
    assert.equal(keys.length, 43, `Expected 43 config sections, got ${keys.length}: ${keys.join(', ')}`);
  });
});
