...
describe('Phase 38 — Version & Config', () => {
  ...
  it('P38-14: /api/health version is 0.34.0', async () => {
    const res = await fetch(BASE_URL + '/api/health');
    const data = await res.json();
    assert.strictEqual(data.version, '0.34.0');
  });

  it('P38-15: /api/docs version is 0.34.0', async () => {
...
describe('Phase 38 — Routes', () => {
  ...
  it('P38-26: /api/docs returns 92 routes', async () => {
    const res = await fetch(BASE_URL + '/api/docs');
    const data = await res.json();
    assert.strictEqual(data.total, 92, `expected 92 routes, got ${data.total}`);
  });
