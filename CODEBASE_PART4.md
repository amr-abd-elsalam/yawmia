...
> Auto-generated: 2026-05-18T20:59:15.135Z
...
<body>
  ...
  <div class="admin-container">
    ...
    <div id="dashboard" class="hidden">
      <!-- Phase 56 — Admin Dashboard IA Tabs -->
      <nav class="admin-tabs" id="adminTabs" aria-label="أقسام لوحة التحكم">
        <button class="admin-tab admin-tab--active" data-admin-tab="overview">نظرة عامة</button>
        <button class="admin-tab" data-admin-tab="marketplace">السوق</button>
        <button class="admin-tab" data-admin-tab="trust">الثقة والأمان</button>
      ...
      <!-- Phase 54 — Production Readiness -->
      <div class="admin-section" id="productionReadinessSection">
        <div class="admin-section__header">
          <h2>🚦 جاهزية الإنتاج</h2>
          <button class="refresh-btn" onclick="AdminApp.loadProductionReadiness()">تحديث</button>
        </div>
        <div id="productionReadinessSummary" class="analytics-grid">
          <p style="color: var(--color-text-muted); text-align: center;">جاري التحميل...</p>
        </div>
        <div id="productionReadinessChecks"></div>
      </div>

      <!-- Phase 54 — Instance Mode + Process Locks -->
      <div class="admin-section" id="instanceModeSection">
        <div class="admin-section__header">
          <h2>🧩 وضع التشغيل والأقفال</h2>
          <button class="refresh-btn" onclick="AdminApp.loadInstanceOps()">تحديث</button>
        </div>
        <div id="instanceModeInfo">
          <p style="color: var(--color-text-muted); text-align: center;">جاري التحميل...</p>
        </div>
      ...
      <div class="admin-section" id="schedulerSection">
        <div class="admin-section__header">
          <h2>⏱ سجل الجدولة</h2>
          <button class="refresh-btn" onclick="AdminApp.loadSchedulers()">تحديث</button>
        </div>
      ...
      <!-- Phase 54 — Ops SLO / Rollups -->
      <div class="admin-section" id="opsSloSection">
        <div class="admin-section__header">
          <h2>📈 SLO التشغيل</h2>
      ...
      <div class="admin-section" id="restoreDrillsSection">
        <div class="admin-section__header">
          <h2>🧪 اختبار استعادة النسخ الاحتياطي</h2>
          <button class="refresh-btn" onclick="AdminApp.loadRestoreDrills()">تحديث</button>
        </div>
        <button class="btn btn--primary btn--sm" onclick="AdminApp.runBackupRestoreDrill()">تشغيل Restore Drill</button>
        <div id="restoreDrillsTable" style="margin-block-start:1rem;">
          <p style="color: var(--color-text-muted); text-align: center;">جاري التحميل...</p>
        </div>
      </div>

      <!-- Phase 54 — Incidents -->
      <div class="admin-section" id="incidentsSection">
        <div class="admin-section__header">
          <h2>🚨 سجل الحوادث التشغيلية</h2>
          <button class="refresh-btn" onclick="AdminApp.loadIncidents()">تحديث</button>
        </div>
      ...
      <!-- Phase 54 — Maintenance Mode -->
      <div class="admin-section" id="maintenanceSection">
        <div class="admin-section__header">
          <h2>🛠 وضع الصيانة</h2>
          <button class="refresh-btn" onclick="AdminApp.loadMaintenanceMode()">تحديث</button>
        </div>
      ...
      <!-- Phase 56 — Marketplace Intelligence -->
      <div class="admin-section" id="marketplaceIntelligenceSection" data-admin-tab-panel="marketplace">
        <div class="admin-section__header">
          <h2>🧠 ذكاء السوق</h2>
        ...
        <div style="display:flex;gap:0.5rem;flex-wrap:wrap;margin-block:1rem;">
          <button class="btn btn--primary btn--sm" onclick="AdminApp.runMarketplaceIntelligenceRollup()">تحديث Rollup</button>
          <button class="btn btn--ghost btn--sm" onclick="AdminApp.loadSearchAnalytics()">تحليلات البحث</button>
          <button class="btn btn--ghost btn--sm" onclick="AdminApp.loadZeroResultSearches()">بحث بدون نتائج</button>
          <button class="btn btn--ghost btn--sm" onclick="AdminApp.loadActivationFunnel()">Activation Funnel</button>
          <button class="btn btn--ghost btn--sm" onclick="AdminApp.loadNotificationConversions()">تحويلات الإشعارات</button>
          <button class="btn btn--ghost btn--sm" onclick="AdminApp.loadWorkroomAdoption()">Workroom Adoption</button>
          <button class="btn btn--ghost btn--sm" onclick="AdminApp.loadPaymentDisputeAnalytics()">نزاعات الدفع</button>
          <button class="btn btn--ghost btn--sm" onclick="AdminApp.loadMatchingQuality()">جودة المطابقة</button>
        </div>

        <div id="marketplaceIntelligenceDetails">
          <p style="color: var(--color-text-muted); text-align: center;">اختار تقرير من الأزرار أعلاه</p>
        </div>
      </div>
      ...
      <!-- Phase 53 — Predictive Signal Precision -->
      <div class="admin-section" id="predictivePrecisionSection">
        <div class="admin-section__header">
          <h2>🧪 دقة إشارات المخاطر</h2>
          <div style="display:flex;gap:0.5rem;flex-wrap:wrap;">
            <button class="refresh-btn" onclick="AdminApp.loadPredictivePrecision()">تحديث</button>
            <button class="btn btn--warning btn--sm" onclick="AdminApp.runPredictiveSignalRetention()">تشغيل Retention</button>
          </div>
        </div>

        <p style="color:var(--color-text-muted);font-size:0.9rem;margin-block-end:1rem;">
          تقيس هذه اللوحة جودة إشارات المخاطر التنبؤية: confirmed مقابل false positives.
          لا يوجد auto-ban بناءً على predictive signals.
        </p>

        <div id="predictivePrecisionMetrics" class="analytics-grid">
          <p style="color: var(--color-text-muted); text-align: center;">جاري التحميل...</p>
        </div>
      </div>
      ...
      <!-- Phase 53 — Trust Score V2 Calibration -->
      <div class="admin-section" id="trustCalibrationSection">
        <div class="admin-section__header">
          <h2>🎯 معايرة مؤشر الثقة V2</h2>
          <div style="display:flex;gap:0.5rem;flex-wrap:wrap;">
            <button class="refresh-btn" onclick="AdminApp.loadTrustCalibrationDashboard()">تحديث</button>
            <button class="btn btn--primary btn--sm" onclick="AdminApp.runTrustSnapshotBatch()">Snapshot Batch</button>
            <button class="btn btn--warning btn--sm" onclick="AdminApp.runTrustCalibrationReport()">تقرير معايرة</button>
          </div>
        </div>

        <p style="color:var(--color-text-muted);font-size:0.9rem;margin-block-end:1rem;">
          هذه اللوحة تقيس مدى تطابق Trust Score V2 مع outcomes الفعلية مثل الحضور، النزاعات، الإلغاءات، والتقييمات.
          لا يتم تعديل الأوزان تلقائياً في Phase 53.
        </p>

        <div id="trustCalibrationMetrics" class="analytics-grid">
          <p style="color: var(--color-text-muted); text-align: center;">جاري التحميل...</p>
        </div>

        <div id="trustCalibrationWarnings">
          <p style="color: var(--color-text-muted); text-align: center;">جاري التحميل...</p>
        </div>

        <hr class="section-divider">

        <h3 style="margin-block-end:0.75rem;font-size:1rem;color:var(--color-text);">آخر Snapshots</h3>
        <div id="trustSnapshotsTable">
          <p style="color: var(--color-text-muted); text-align: center;">جاري التحميل...</p>
        </div>

        <div id="trustCalibrationActionResult" style="margin-block-start:0.75rem;"></div>
      </div>
  ...
  <div id="abuseFlagReviewModal" class="ym-modal-overlay hidden" role="dialog" aria-modal="true" aria-labelledby="flagReviewModalTitle" style="display:none;">
    <div class="ym-modal-card" style="max-width:600px;">
      <h3 class="ym-modal-title" id="flagReviewModalTitle">📋 مراجعة إشارة إساءة</h3>
      <div id="flagReviewDetails" style="margin-block-end:1rem;font-size:0.9rem;background:var(--color-surface-2);padding:0.75rem;border-radius:var(--radius-sm);"></div>
      <div style="margin-block-end:1rem;">
        <strong style="font-size:0.9rem;">سجل المراجعة:</strong>
        <div id="flagReviewHistory" style="max-height:200px;overflow-y:auto;margin-block-start:0.5rem;font-size:0.85rem;"></div>
      </div>
      <textarea id="flagReviewNote" class="form-input form-textarea" rows="3" placeholder="ملاحظة (اختياري — أقصى 500 حرف)..." maxlength="500" style="margin-block-end:1rem;"></textarea>
      <div class="ym-modal-error" id="flagReviewError" style="display:none;"></div>
      <div class="ym-modal-actions" style="flex-wrap:wrap;gap:0.5rem;justify-content:flex-start;">
        <button class="btn btn--ghost btn--sm" data-decision="dismissed">رفض الإشارة</button>
        <button class="btn btn--warning btn--sm" data-decision="snoozed" data-snooze-days="7">تأجيل 7 أيام</button>
        <button class="btn btn--warning btn--sm" data-decision="snoozed" data-snooze-days="30">تأجيل 30 يوم</button>
        <button class="btn btn--primary btn--sm" data-decision="warning">📨 إرسال تحذير</button>
        <button class="btn ym-modal-btn--danger btn--sm" data-decision="actioned">🚫 حظر</button>
        <button class="btn btn--ghost btn--sm" id="closeFlagReviewModal">إغلاق</button>
      </div>
    </div>
  </div>
...
.profile-task-item:focus-visible {
  border-color: var(--color-primary);
  background: rgba(37, 99, 235, 0.08);
  transform: translateY(-1px);
}
...
.profile-task-priority--critical {
  color: #fff;
  background: var(--color-error);
}

.profile-task-priority--high {
  color: #000;
  background: var(--color-warning);
}

.profile-task-priority--medium {
  color: #fff;
  background: var(--color-primary);
}

.profile-task-priority--low {
  color: var(--color-text-muted);
  background: rgba(139, 143, 163, 0.15);
  border: 1px solid var(--color-border);
}

.profile-task-item__body {
  flex: 1;
  min-width: 0;
  display: flex;
  flex-direction: column;
...
.profile-task-item__body strong {
  font-size: 0.92rem;
  color: var(--color-text);
}

.profile-task-item__body small {
  font-size: 0.78rem;
  color: var(--color-text-muted);
  line-height: 1.5;
}

.profile-task-item__arrow {
  color: var(--color-text-muted);
  font-size: 1.1rem;
  flex-shrink: 0;
}
...
/* ═══ Phase 53 — Workroom Search + Read Receipts ═══ */
.workroom-search-box {
  display: flex;
  gap: 0.5rem;
  margin-block-end: 1rem;
...
.workroom-search-box input {
  flex: 1;
}

.workroom-search-results {
  display: flex;
  flex-direction: column;
  gap: 0.6rem;
}

.workroom-search-meta {
  color: var(--color-text-muted);
  font-size: 0.8rem;
  margin-block-end: 0.5rem;
}

.workroom-search-result {
  background: var(--color-surface-2);
  border: 1px solid var(--color-border);
  border-radius: var(--radius-sm);
  padding: 0.75rem 0.9rem;
...
.workroom-search-result__text {
  color: var(--color-text);
  font-size: 0.9rem;
  line-height: 1.6;
}

.workroom-search-result__meta {
  color: var(--color-text-muted);
  font-size: 0.75rem;
  margin-block-start: 0.3rem;
}
...
.read-receipt--unread,
.read-receipt--unknown {
  color: var(--color-text-muted);
}

.workroom-timeline-filter {
  margin-block-end: 0.75rem;
}
...
/* ═══ Phase 53 — Workroom Pins + Checklist ═══ */
.workroom-pin-list {
  display: flex;
  flex-direction: column;
  gap: 0.75rem;
...
.workroom-pin-card__text {
  color: var(--color-text);
  font-size: 0.92rem;
  line-height: 1.7;
}

.workroom-pin-card__meta {
  color: var(--color-text-muted);
  font-size: 0.75rem;
  margin-block-start: 0.35rem;
}
...
.workroom-checklist-create {
  display: flex;
  gap: 0.5rem;
  margin-block-end: 0.75rem;
}

.workroom-checklist-create input {
  flex: 1;
}

.workroom-checklist-items {
  display: flex;
  flex-direction: column;
  gap: 0.5rem;
}
...
.workroom-checklist-item input[type="checkbox"] {
  width: 1.1rem;
  height: 1.1rem;
  accent-color: var(--color-success);
}

.workroom-checklist-item--done span {
  text-decoration: line-through;
  color: var(--color-text-muted);
}
...
/* ═══ Phase 53 — Workroom Attachments + Summary Cards ═══ */
.workroom-summary-grid {
  display: grid;
...
.workroom-summary-card__value {
  color: var(--color-primary);
  font-weight: 800;
  font-size: 1.2rem;
}

.workroom-summary-card__label {
  color: var(--color-text-muted);
  font-size: 0.75rem;
  margin-block-start: 0.25rem;
}

.workroom-attachment-row {
  display: flex;
  flex-direction: column;
  gap: 0.35rem;
  margin-block-end: 0.75rem;
...
/* ═══ Phase 53 — Trust Calibration Admin UI ═══ */
...
.scheduler-status-badge--queued,
.scheduler-status-badge--deduped {
  color: #60a5fa;
  background: rgba(37, 99, 235, 0.14);
  border-color: rgba(37, 99, 235, 0.35);
}

.scheduler-status-badge--failed {
  color: var(--color-error);
  background: rgba(239, 68, 68, 0.14);
  border-color: rgba(239, 68, 68, 0.35);
}

.scheduler-status-badge--completed,
.scheduler-status-badge--registered {
  color: var(--color-success);
  background: rgba(34, 197, 94, 0.14);
  border-color: rgba(34, 197, 94, 0.35);
}

.scheduler-status-badge--skipped {
  color: var(--color-text-muted);
...
.maintenance-banner-admin--active {
...
.marketplace-intelligence-card__value {
  font-size: 1.45rem;
  font-weight: 800;
  color: var(--color-primary);
  line-height: 1.2;
...
.marketplace-intelligence-card__label {
  font-size: 0.82rem;
  color: var(--color-text-muted);
  margin-block-start: 0.35rem;
}

.zero-result-query-card {
  padding: 0.75rem 0.9rem;
  background: var(--color-surface-2);
  border: 1px solid var(--color-border);
  border-inline-start: 4px solid var(--color-warning);
...
(function () {
  ...
  function renderProfileTasks(mount, tasks, completionScore) {
    ...
    visible.forEach(function (task) {
      html +=
        '<a class="profile-task-item" data-task-id="' + YawmiaUtils.escapeHtml(task.id || '') + '" href="' + YawmiaUtils.escapeHtml(task.url || '/profile.html') + '">' +
